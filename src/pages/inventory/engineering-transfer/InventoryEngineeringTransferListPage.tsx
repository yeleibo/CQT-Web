...
const InventoryEngineeringTransferListPage: React.FC = () => {
  ...
  const columns: ProColumns<InventoryEngineeringTransferListDto>[] = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <div
            style={{
              backgroundColor: getInventoryEngineeringTransferStatusColor(record.status, false),
              color: getInventoryEngineeringTransferStatusColor(record.status, true),
              padding: '5px',
            }}
          >
    ...
    },
    {
      ...
      render: (_, record) => {
        if (
          getInventoryEngineeringTransferStatusDescription(record.status) === '已完成' ||
          getInventoryEngineeringTransferStatusDescription(record.status) === '执行中' ||
  ...
  const fetchInventoryEngineeringTransferData = async (): Promise<{
    [sheetName: string]: InventoryEngineeringTransferListDto[];
  }> => {
    let params: InventoryEngineeringTransfersQueryParam = {
      pageSize: 999999999,
    };
    let r = await EngineeringTransferService.list(params);
    return { 工程调剂单据: r['data'] };
  };

  const fetchInventoryEngineeringTransferGoodData = async (): Promise<{
    [sheetName: string]: InventoryEngineeringTransferGoodExcelDetail[];
  }> => {
    let params: InventoryEngineeringTransfersQueryParam = {
      pageSize: 999999999,
    };
    let r = await EngineeringTransferService.getGoodExcel(params);
    return { 工程调剂单据: r };
  };
      ...
      <ProTable<InventoryEngineeringTransferListDto, InventoryEngineeringTransfersQueryParam>
        ...
        headerTitle={
          <div>
            ...
            <ExportButton
              fileName={'工程调剂单据'}
              headers={engineeringTransferHeaders}
              fetchData={fetchInventoryEngineeringTransferData}
              buttonName={'导出单据'}
            ></ExportButton>
            <ExportButton
              fileName={'工程调剂单据'}
              headers={engineeringTransferGoodHeader}
              fetchData={fetchInventoryEngineeringTransferGoodData}
              buttonName={'导出物资'}
            ></ExportButton>
          </div>
        }
...
export default InventoryEngineeringTransferListPage;
