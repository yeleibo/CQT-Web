...
const FatEndBox: React.FC<FatEndBoxProps> = ({ chaoqianBoxPorts, onChange, codeChangeFunction }) => {
  ...
  const handleCodeChange = (code: string | null, name: string) => {
    if (code === null || checkBoxPortCableCode(code, "FatEndBox", name)) {
      onChange?.(name, code);
    } else {
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', right: isDesktop ? 0 : 10, top: 10, zIndex: 1 }}>
        <InfoCircleOutlined 
          style={{ color: '#999' }} 
